...
function tempTest() {
  ...
  urls.forEach(function(url) {
    ...
    docsAddText("URL : " + url)
    docsAddText("제목 : " + json_result.title)
    docsAddText("내용 : " + json_result.content)
    docsAddText("이미지 : " + json_result.images)
  })
}
...
function getContentKinNaver() {
  ...
  Logger.log(url)
  var response = UrlFetchApp.fetch("https://kin.naver.com/qna/detail.naver?d1id=4&dirId=4010301&docId=474881826");
  var htmlSource = response.getContentText();
  docsAddText(htmlSource)
...
function extractContentKinNaver(url) {
    ...
    // 제목 추출
    var titlePattern = /<div class="endTitleSection">[\s\S]*?<span class="iconQuestion">[\s\S]*?<\/span>[\s\S]*?<\/span>([\s\S]*?)<\/div>/;
    var titleMatch = titlePattern.exec(html);
    var title = titleMatch ? titleMatch[1].trim() : "제목을 찾을 수 없습니다.";

    // 본문 내용 추출
    var contentPattern = /<div class="questionDetail[^"]*"[^>]*>([\s\S]*?)<\/div>/;
    var contentMatch = contentPattern.exec(html);
    var content = contentMatch ? contentMatch[1].trim().replace(/<br>/g, "\n") : "내용을 찾을 수 없습니다.";
